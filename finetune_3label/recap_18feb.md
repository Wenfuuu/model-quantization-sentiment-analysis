...
[insert image - comparison table PyTorch fake quantization]
...
[insert image - ONNX comparison table GPU]
...
- ❌ **Latency SLOWER**: FP32 3.89 ms → INT8 19.92 ms (+411.79%)
  ...
  - Seperti orang kerja di kantor tapi alat kerjanya ada di rumah → harus pulang-pergi terus = buang waktu
  - Kesimpulan: INT8 di GPU malah **lebih lambat 5x** karena overhead perpindahan data

**CPU Deployment:**

[insert image - ONNX comparison table CPU]
...
2. **INT8 on GPU Performance ❌**
   - INT8 ONNX 5x slower di GPU dibanding FP32
   - 186 Memcpy nodes = CPU↔GPU transfer overheadmenga
   - CUDA lacks native INT8 support untuk transformers
   - **Recommendation: JANGAN gunakan INT8 di GPU**
