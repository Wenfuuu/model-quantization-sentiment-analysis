...
from scripts.run_xai import interactive_menu as xai_menu, run_xai_experiment, run_xai_diagnostics, run_ste_ig_analysis, run_lime_attribution, run_shap_attribution, run_occlusion_attribution, run_ste_calibration, run_random_baselines, run_cross_seed_verification, run_stability_analysis
...
def run_xai():
    ...
    if experiment_key == "ste_cal":
        run_ste_calibration()
        print_section("STE CALIBRATION COMPLETED")
        return

    if experiment_key == "rand_base":
        run_random_baselines()
        print_section("RANDOM BASELINES COMPLETED")
        return

    if experiment_key == "cross_seed":
        run_cross_seed_verification()
        print_section("CROSS-SEED VERIFICATION COMPLETED")
        return
