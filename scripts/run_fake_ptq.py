...
def run_fake_ptq_experiment(version_key, num_runs_override=None):
    ...
    print_section(f"FAKE QUANTIZATION EXPERIMENT: {version_key}")
    ...
    print_section("FAKE QUANTIZATION - FP16 (Simulated Half Precision)")

    ptq_quantizer = PTQQuantizer(base_model.model)
    model_fake_fp16, fake_fp16_time = ptq_quantizer.quantize_fake_fp16()
    print(f"Fake FP16 Quantization completed in {fake_fp16_time:.2f} seconds")

    fake_fp16_path = output_dir / "model_fake_fp16.pth"
    save_quantized_model(model_fake_fp16, fake_fp16_path)
    fake_fp16_size_mb = get_model_size(fake_fp16_path)
    ...
    print_section("FAKE QUANTIZATION - INT8 (Simulated 8-bit)")

    model_fake_int8, fake_int8_time = ptq_quantizer.quantize_fake_int8()
    print(f"Fake INT8 Quantization completed in {fake_int8_time:.2f} seconds")

    fake_int8_path = output_dir / "model_fake_int8.pth"
    save_quantized_model(model_fake_int8, fake_int8_path)
    fake_int8_size_mb = get_model_size(fake_int8_path)
    ...
    print_section("FAKE QUANTIZATION - INT4 (Simulated 4-bit)")

    model_fake_int4, fake_int4_time = ptq_quantizer.quantize_fake_int4()
    print(f"Fake INT4 Quantization completed in {fake_int4_time:.2f} seconds")

    fake_int4_path = output_dir / "model_fake_int4.pth"
    save_quantized_model(model_fake_int4, fake_int4_path)
    fake_int4_size_mb = get_model_size(fake_int4_path)
    ...
    print_section("FAKE QUANTIZATION COMPARISON SUMMARY")

    df_comparison = generate_comparison_report(
        fp32_results, fp16_results, int8_results, int4_results,
        fp32_size_mb, fake_fp16_size_mb, fake_int8_size_mb, fake_int4_size_mb
    )
    ...
    print(f"\nFake FP16 vs FP32 Consistency: {consistency['fp16']*100:.1f}%")
    print(f"Fake INT8 vs FP32 Consistency: {consistency['int8']*100:.1f}%")
    print(f"Fake INT4 vs FP32 Consistency: {consistency['int4']*100:.1f}%")

    print_section("PREDICTION DIVERGENCES")

    divergences = []
    precisions_list = ["fp32", "fake_fp16", "fake_int8", "fake_int4"]
    all_preds = {
        "fp32": fp32_results["predictions"],
        "fake_fp16": fp16_results["predictions"],
        "fake_int8": int8_results["predictions"],
        "fake_int4": int4_results["predictions"]
    }
    ...
    print(f"\nFake INT8 vs FP32 Latency (t-test)")
    ...
    print(f"\nFake INT4 vs FP32 Latency (t-test)")
    ...
    print(f"  Fake INT8 vs FP32 p-value: {conf_comp_int8['p_value']:.6f}")
    ...
    return {
        ...
        "fp16_size_mb": fake_fp16_size_mb,
        "int8_size_mb": fake_int8_size_mb,
        "int4_size_mb": fake_int4_size_mb,
    }


def interactive_menu():
    print("\n" + "=" * 60)
    print("  FAKE QUANTIZATION PTQ EXPERIMENT RUNNER")
    print("=" * 60)
